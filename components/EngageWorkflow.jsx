...
const ValidationErrorsModal = ({ isOpen, onClose, errors }) => {
    if (!isOpen) return null;
    return (
        <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
...
// --- ALIGNMENT ALGORITHM ---
const calculateAutoLayout = (nodes, edges, direction) => {
    // 1. Hierarchy Calc
    const adjacency = {};
    const incoming = {};
...
const WorkflowBuilder = ({ onDirtyChange, onShowAnalytics }) => {
    // Graph State
    const [nodes, setNodes] = useState(INITIAL_NODES_GRAPH);
    const [edgesList, setEdgesList] = useState(INITIAL_EDGES_GRAPH);
    const [layoutDirection, setLayoutDirection] = useState('HORIZONTAL');

    // History State
    const [history, setHistory] = useState([{ nodes: INITIAL_NODES_GRAPH, edges: INITIAL_EDGES_GRAPH }]);
    ...
    const [configCriteriaId, setConfigCriteriaId] = useState(null);
    ...
    const handleToggleAutomation = (criteriaId, enabled) => {
    ...
    const isConnectionValid = (sourceId, targetId) => {
            ...
            >
                ...
                >
                    <svg className="absolute top-0 left-0 w-full h-full pointer-events-none" style={{ overflow: 'visible' }}>
                        {edges.map((edge, i) => <BezierEdge key={i} {...edge} direction={layoutDirection} />)}
                        {connectingNodeId && (
                            <defs>
...
export const EngageWorkflow = ({ activeView = 'BUILDER' }) => {
    ...
    const [analyticsRoundType, setAnalyticsRoundType] = useState('Screening');
