...
import { SourceAI } from './SourceAI.jsx';
  ...
  <div className="p-8 h-full overflow-y-auto">
    ...
    <div className="border rounded-lg overflow-hidden">
      <table className="w-full text-sm text-left">
        ...
        <tbody className="divide-y divide-slate-100 bg-white dark:bg-slate-700">
  ...
  <div className="p-8 h-full overflow-y-auto">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {['LinkedIn Recruiter', 'Indeed', 'ZipRecruiter', 'Monster', 'Dice', 'CareerBuilder'].map(platform => (
        <div key={platform} className="border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded-xl p-6 flex flex-col items-center text-center hover:shadow-md transition-shadow bg-white dark:bg-slate-700">
          <div className="w-12 h-12 bg-slate-100 dark:bg-slate-700 rounded-full mb-4 flex items-center justify-center font-bold text-slate-500 text-lg shadow-sm">
            {platform[0]}
          </div>
          ...
          <button className="px-4 py-2 border border-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-700 dark:hover:bg-slate-700 dark:bg-slate-700 text-slate-700 dark:text-slate-200 transition-colors w-full">Connect</button>
        </div>
      ))}
    </div>
  </div>
...
const JobDescriptionPlaceholder = () => (
  <div className="p-8 h-full overflow-y-auto max-w-5xl">
    ...
    <div className="bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-700 dark:border-slate-700 rounded-xl p-8 shadow-sm prose prose-sm text-slate-600 dark:text-slate-300 max-w-none">
      <h4 className="text-slate-800 dark:text-slate-200 font-bold text-lg mb-2">About the Role</h4>
      ...
      <h4 className="text-slate-800 dark:text-slate-200 font-bold text-lg mb-2">Responsibilities</h4>
      ...
      <h4 className="text-slate-800 dark:text-slate-200 font-bold text-lg mb-2">Requirements</h4>
...
export const CampaignSourceAI = ({ hideSidebar = false, activeView = 'ATTACH' }) => {
  ...
  return (
    <div className="flex h-full bg-slate-50/30">
      ...
      {!hideSidebar && (
        <div className="w-64 border-r border-slate-200 dark:border-slate-700 dark:border-slate-700 bg-slate-50 dark:bg-slate-700 flex flex-col gap-1 p-3 shrink-0">
          <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 px-3 mt-2">Sourcing Tools</div>
          <button onClick={() => setView('ATTACH')} className={`flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-all ${view === 'ATTACH' ? 'bg-white dark:bg-slate-700 text-indigo-600 shadow-sm ring-1 ring-slate-100' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100'}`}>
            <PlusCircle size={18} /> Attach People
          </button>
          <button onClick={() => setView('PROFILES')} className={`flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-all ${view === 'PROFILES' ? 'bg-white dark:bg-slate-700 text-indigo-600 shadow-sm ring-1 ring-slate-100' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100'}`}>
            <Users size={18} /> Attached Profiles
            <span className="ml-auto bg-slate-200 text-slate-600 dark:text-slate-300 text-xs px-1.5 py-0.5 rounded-full">4</span>
          </button>
          <button onClick={() => setView('INTEGRATIONS')} className={`flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-all ${view === 'INTEGRATIONS' ? 'bg-white dark:bg-slate-700 text-indigo-600 shadow-sm ring-1 ring-slate-100' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100'}`}>
          ...
          <button onClick={() => setView('JD')} className={`flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-all ${view === 'JD' ? 'bg-white dark:bg-slate-700 text-indigo-600 shadow-sm ring-1 ring-slate-100' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100'}`}>
      ...
      {/* Main Content */}
      <div className="flex-1 overflow-hidden relative bg-white dark:bg-slate-700">
