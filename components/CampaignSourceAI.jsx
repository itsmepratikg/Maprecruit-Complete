...
import {
  PlusCircle, Users, Link as LinkIcon, FileText
} from 'lucide-react';
...
export const CampaignSourceAI = ({ hideSidebar = false, activeView = 'ATTACH' }) => {
  ...
  return (
    <div className="flex h-full bg-slate-50/30 dark:bg-slate-900/30">
      ...
      {!hideSidebar && (
        <div className="w-64 border-r border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex flex-col gap-1 p-3 shrink-0 transition-colors">
          <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2 px-3 mt-2">Sourcing Tools</div>
          <button onClick={() => setView('ATTACH')} className={`flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-all ${view === 'ATTACH' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm ring-1 ring-slate-100 dark:ring-slate-600' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'}`}>
            <PlusCircle size={18} /> Attach People
          </button>
          <button onClick={() => setView('PROFILES')} className={`flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-all ${view === 'PROFILES' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm ring-1 ring-slate-100 dark:ring-slate-600' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'}`}>
            <Users size={18} /> Attached Profiles
            <span className="ml-auto bg-slate-200 dark:bg-slate-600 text-slate-600 dark:text-slate-300 text-xs px-1.5 py-0.5 rounded-full">4</span>
          </button>
          <button onClick={() => setView('INTEGRATIONS')} className={`flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-all ${view === 'INTEGRATIONS' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm ring-1 ring-slate-100 dark:ring-slate-600' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'}`}>
            <LinkIcon size={18} /> Integrations
          </button>
          <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2 px-3 mt-4">Job Details</div>
          <button onClick={() => setView('JD')} className={`flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-all ${view === 'JD' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm ring-1 ring-slate-100 dark:ring-slate-600' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'}`}>
