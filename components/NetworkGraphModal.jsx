...
export const NetworkGraphModal = ({ isOpen, onClose, initialRoundType = 'Screening' }) => {
    ...
    useEffect(() => {
        ...
        else if (['Announcement', 'Screening', 'Interview', 'Survey'].includes(initialRoundType)) type = initialRoundType;
    ...
    useEffect(() => {
        ...
        const highlightFlow = (startNode, extraLink = null, extraNode = null) => {
            ...
            // Highlight active
            svg.selectAll(".sankey-node").filter((n) => activeNodes.has(n)).attr("opacity", 1);
            svg.selectAll(".sankey-link").filter((l) => activeLinks.has(l)).attr("stroke-opacity", 0.7);
            svg.selectAll(".sankey-label").filter((n) => activeNodes.has(n)).attr("opacity", 1);
        };
        ...
        link.append("path")
            ...
            .attr("stroke", (d) => getColor(d.target.name)) // Link color matches target node
