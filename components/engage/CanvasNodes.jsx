...
export const StartNode = ({ node, onClick, onStartConnect, isConnecting, layoutDirection = 'HORIZONTAL' }) => {
  const outputClass = layoutDirection === 'HORIZONTAL'
...
export const NodeCard = ({ node, onSelect, onEdit, onDelete, onStartConnect, onEndConnect, isSelected, isConnecting, isValidTarget, layoutDirection = 'HORIZONTAL', onShowAnalytics }) => {
  ...
  const config = NODE_TYPES[node.type] || NODE_TYPES.ANNOUNCEMENT;
...
export const BezierEdge = ({ start, end, label, direction = 'HORIZONTAL' }) => {
  let path = '';
