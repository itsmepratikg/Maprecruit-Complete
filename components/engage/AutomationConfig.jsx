...
export const AutomationConfig = ({ isEnabled, onToggle }) => {
    return (
        <div className="flex flex-col h-full bg-white">
            {/* Status Bar */}
            <div className="px-6 py-3 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-700 flex justify-between items-center shrink-0">
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        <span className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white dark:bg-slate-700 shadow ring-0 transition duration-200 ease-in-out ${isEnabled ? 'translate-x-5' : 'translate-x-0'}`} />
                    </button>
                </div>
            </div>
            ...
            <div className="p-8 text-center space-y-4 flex-1 overflow-y-auto">
                <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-2 ${isEnabled ? 'bg-green-100 text-green-600' : 'bg-slate-100 dark:bg-slate-700 text-slate-400'}`}>
                    <GitBranch size={32} />
                </div>
                ...
                {!isEnabled && (
                    <div className="bg-slate-50 dark:bg-slate-700 p-4 rounded-lg border border-slate-200 dark:border-slate-700 text-left text-xs text-slate-500 mt-4 mx-auto max-w-xs">
