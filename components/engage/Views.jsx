...
export const CandidateTracking = () => {
   ...
   return (
      <div className="flex h-full bg-white">
         <div className="w-80 border-r border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700 flex flex-col">
            <div className="p-4 border-b border-slate-200 dark:border-slate-700 bg-white">
               <div className="relative">
                  <Search className="absolute left-3 top-2.5 text-slate-400" size={14} />
                  <input type="text" placeholder="Search..." className="w-full pl-9 pr-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm bg-slate-50 dark:bg-slate-700 focus:bg-white dark:bg-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none transition-all" />
               </div>
            </div>
            <div className="flex-1 overflow-y-auto">
               {candidates.map(c => (
                  <div
                     ...
                     className={`p-4 border-b border-slate-100 dark:border-slate-700 cursor-pointer hover:bg-white dark:bg-slate-700 dark:hover:bg-slate-700 transition-colors group ${selectedCandidate.id === c.id ? 'bg-white dark:bg-slate-700 border-l-4 border-l-indigo-600 shadow-sm' : ''}`}
                  >
                     ...
                     <div className="mt-2 flex justify-between items-center">
                        <span className="text-[10px] bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-1.5 py-0.5 rounded border border-slate-200">{c.stage}</span>
         ...
         <div className="flex-1 flex flex-col overflow-hidden bg-white">
            <div className="p-8 pb-4 border-b border-slate-100">
               <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="flex gap-2">
                     <button className="px-4 py-2 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50">View Profile</button>
               ...
               <div className="relative flex items-center justify-between px-4 pb-4">
                  ...
                  {steps.map((step, i) => (
                     <div key={i} className="flex flex-col items-center bg-white dark:bg-slate-700 px-4">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 mb-2 transition-all ${step.status === 'completed' ? 'bg-green-500 border-green-500 text-white' :
                           step.status === 'active' ? 'bg-white dark:bg-slate-700 border-indigo-600 text-indigo-600 shadow-lg scale-110' :
                              'bg-white dark:bg-slate-700 border-slate-200 dark:border-slate-700 text-slate-300'
                           }`}>
            ...
            <div className="flex-1 overflow-y-auto p-8 bg-slate-50/50 space-y-6">
               {/* Active Action Card */}
               <div className="bg-white dark:bg-slate-700 rounded-xl border border-indigo-100 shadow-sm overflow-hidden">
                  ...
                  <div className="p-6">
                     <div className="flex items-start gap-4 mb-6">
                        <div className="w-12 h-12 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-400"><User size={24} /></div>
                        <div>
                           ...
                           <div className="mt-2 flex gap-2">
                              <span className="text-[10px] bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded text-slate-500">Video Call</span>
                              <span className="text-[10px] bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded text-slate-500">Live Coding</span>
                           </div>
                        </div>
                     </div>
                     <div className="flex gap-3 pt-4 border-t border-slate-50">
                        ...
                        <button className="flex-1 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 py-2.5 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-700 dark:hover:bg-slate-700 transition-colors">Nudge Interviewer</button>
                     </div>
                  </div>
               </div>
               ...
               <div>
                  ...
                  <div className="space-y-3">
                     <div className="bg-white dark:bg-slate-700 rounded-lg p-4 border border-slate-200 dark:border-slate-700 flex justify-between items-center shadow-sm">
                        <div className="flex items-center gap-3">
...
export const InterviewRoom = () => (
   <div className="flex h-full bg-slate-100 dark:bg-slate-700 overflow-hidden">
      {/* Left: Context */}
      <div className="w-1/2 bg-slate-600 flex flex-col border-r border-slate-700">
         ...
         <div className="flex-1 bg-slate-500 p-8 overflow-y-auto">
            <div className="bg-white dark:bg-slate-700 shadow-2xl min-h-[800px] w-full max-w-2xl mx-auto rounded-sm p-12 relative">
      ...
      {/* Right: Scorecard */}
      <div className="w-1/2 bg-white dark:bg-slate-700 flex flex-col">
         <div className="h-14 border-b border-slate-200 dark:border-slate-700 px-6 flex items-center justify-between shrink-0 bg-white">
            <div>
         ...
         <div className="flex-1 overflow-y-auto p-8 space-y-8">
            ...
            <div>
               ...
               <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map(n => (
                     <button key={n} className="flex-1 py-2 border border-slate-300 rounded hover:border-indigo-500 hover:bg-indigo-50 text-sm font-medium text-slate-600 dark:text-slate-300 focus:bg-indigo-600 focus:text-white focus:border-indigo-600 transition-colors">{n}</button>
                  ))}
               </div>
            </div>
            ...
            <div>
               ...
               <div className="flex gap-4">
                  <button className="flex-1 py-3 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-green-50 hover:border-green-300 hover:text-green-700 text-slate-500 flex items-center justify-center gap-2 transition-colors"><ThumbsUp size={18} /> Good</button>
                  <button className="flex-1 py-3 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-red-50 hover:border-red-300 hover:text-red-700 text-slate-500 flex items-center justify-center gap-2 transition-colors"><ThumbsDown size={18} /> Poor</button>
               </div>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
               ...
               <textarea className="w-full bg-white dark:bg-slate-700 border border-amber-300 rounded p-3 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 min-h-[80px]" placeholder="Please explain the score..."></textarea>
            </div>
            ...
            <div className="pt-6 border-t border-slate-100">
               ...
               <div className="grid grid-cols-4 gap-3">
                  ...
                  <button className="py-3 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-700 dark:hover:bg-slate-700 transition-colors">No</button>
                  <button className="py-3 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-700 dark:hover:bg-slate-700 transition-colors">Yes</button>
...
export const PanelView = () => (
   <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-700 p-6 overflow-y-auto">
      <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-6">Interview Panel</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         ...
         <div className="bg-white dark:bg-slate-700 p-6 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm flex items-center gap-4">
         ...
         <div className="bg-white dark:bg-slate-700 p-6 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm flex items-center gap-4 flex-col justify-center text-center border-dashed border-gray-300 bg-gray-50 dark:bg-slate-700 hover:bg-gray-100 dark:bg-slate-700 cursor-pointer transition-colors text-gray-400 hover:text-indigo-600">
            <div className="p-2 bg-white dark:bg-slate-700 rounded-full"><span className="text-xl">+</span></div>
